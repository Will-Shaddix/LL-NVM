...
system.loc_mem_ctrlrs = [MemCtrl() for i in range(8)]


if(options.mem_type == "DRAM"):
    loc_ranges = interleave_addresses(DDR4_2400_16x4, 8, 128, 0, '3GiB') # 3GB Local memory spread across 8 HBM Controllers and a DDR channel
    print("size of loc_Ranges: " + str(loc_ranges[0].size) + "    " + str(loc_ranges[0].start) + "  " + str(loc_ranges[0].end))
    print("mhere")
    for i in range (0,8):
        system.loc_mem_ctrlrs[i].dram = DDR4_2400_16x4(range=loc_ranges[i], in_addr_map=False, null=True) # why the dram and dram_2?changed in_addr_map
...
def createLinearTraffic(tgen):
    yield tgen.createLinear(3000000000,            # duration 
                            0,                      # min_addr
                            AddrRange('3GiB').end,  # max_adr
                            64,                     # block_size
